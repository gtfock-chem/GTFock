...
// A KetShellPairList_t can holds 16 ket side shellpairs
...
typedef struct 
{
	...
	FockQuartetInfo_t *fock_info_list;  
} KetShellPairList_t;
...
typedef struct 
{
	...
	KetShellPairList_t *ket_shellpair_lists;  
} ThreadQuartetLists_t;

void init_KetShellPairList(KetShellPairList_t *ket_shellpair_list)
{
	...
	ket_shellpair_list->fock_info_list = (FockQuartetInfo_t *) malloc(sizeof(FockQuartetInfo_t) * 16);
...
void free_KetShellPairList(KetShellPairList_t *ket_shellpair_list)
{
	assert(ket_shellpair_list != NULL);
...
void reset_KetShellPairList(KetShellPairList_t *ket_shellpair_list)
{
	assert(ket_shellpair_list != NULL);
	ket_shellpair_list->num_shellpairs = 0;
}
...
int add_KetShellPair(
	KetShellPairList_t *ket_shellpair_list, int _P, int _Q, 
...
void init_ThreadQuartetLists(ThreadQuartetLists_t *thread_quartet_lists)
{
	assert(thread_quartet_lists != NULL);
	
	thread_quartet_lists->ket_shellpair_lists = (KetShellPairList_t *) malloc(sizeof(KetShellPairList_t) * 8 * 8);  
...
void free_ThreadQuartetLists(ThreadQuartetLists_t *thread_quartet_lists)
{
	assert(thread_quartet_lists != NULL);
...
void reset_ThreadQuartetLists(ThreadQuartetLists_t *thread_quartet_lists, const int _M, const int _N)
{
	assert(thread_quartet_lists != NULL);
